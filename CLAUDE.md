# CLAUDE.md - React & Rust 项目开发规范

本文件是 Claude Code 的行为约束指南，Claude 在协助进行本项目（包含 React 前端和 Rust 后端/桌面端）的开发、重构、测试和调试时，**必须严格遵守**以下规范。

---

## 1. 项目常用指令 (Commands)

根据具体使用的技术栈，Claude 可以运行以下命令进行构建、测试和代码检查：
...
## 5. Claude Code 交互守则 (AI Instructions)

当 Claude Code 在此项目中生成或重构代码时，必须遵守以下操作规范：
